...
namespace XLua.CSObjectWrap {
	...
	public class MinecraftInventoryManagerWrap {
		...
		[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
		private static int _m_SetItem(RealStatePtr L) {
			try {
				ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
...

				{
					...
					Minecraft.ItemsData.ItemType _type;
					translator.Get(L, 3, out _type);
		...
		[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
		private static int _g_get_CurrentItem(RealStatePtr L) {
			try {
				ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
